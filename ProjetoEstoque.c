...
void listartudo(produto *produtos,int i){
	...
	for (g=0;g<i;g++){
		printf("\nCodigo: %i. \tNome: %s. \nPreco: %.2f. \tEstoque: %.0f.\n",produtos[g].cod, produtos[g].nome, produtos[g].preco, produtos[g].qtd);
	}
		printf("\n\n");
		system("pause");
		system("cls");
...
void Pesquisar(produto *produtos, int *i){
	...
	for(g=0;g<*i;g++){
		if(strcmp(produtos[g].nome,busca)==0){
			achou=1;
			printf("\n*****Produto encontrado!*****\n\nCodigo:%i\nProduto:%s\nQuantidade:%.2f\nPreco: %.2f.\nEstoque: %.0f.\n",produtos[g].cod,produtos[g].nome, produtos[g].qtd, produtos[g].preco, produtos[g].qtd);
			break;
		}
	}
		if(achou==0) {
...
void Saida(produto *produtos, int *i){
	...
	for(g=0;g<*i;g++){
		if(strcmp(produtos[g].nome,busca)==0){
			achou=1;
			printf("\nQuantidade atual do produto: %.2f\n",produtos[g].qtd);
			printf("\nInsira a quantidade a remover:\n");
			scanf("%f",&novo);
			do{
			if((produtos[g].qtd-novo)<0){
			printf("Estoque nao pode ficar negativo, digite um valor valido:  ");
			scanf("%f",&novo);
			}
